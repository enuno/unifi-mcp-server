...
- **`Makefile`**: one entry point for the developer workflow. Targets run through `uv run --frozen --extra dev`, so no virtualenv activation is needed (`RUN=` runs the tools directly for pip users). `make ci` runs the blocking CI checks with CI's exact commands (lint, bandit, docstring coverage, pre-commit with CI's skip list, and the 80% coverage gate); `make install` builds `.venv` from `uv.lock`; `make typecheck` runs mypy, which CI does not block on yet. Also `format`, `test`, `audit`, `run`, `inspect`, `docker-build`, `compose-up`/`down`/`logs`, `clean` and `help`.

- **Device migration** (`move_device_to_site`, `migrate_device`, `cancel_device_migration`): move an adopted device to another site on the same controller (`cmd/sitemgr` `move-device`), or hand it to another controller by setting its inform URL (`cmd/devmgr` `migrate`, aborted with `cancel-migrate`). `move-device` needs the target site's 24-hex `_id`, which only the legacy `/api/self/sites` listing carries (`list_sites` returns Integration API UUIDs), so the target is resolved from a short name, UI description or `_id`, and a description shared by several sites is rejected as ambiguous. Each tool finds the device in the source site first (MAC in any format, or device ID), so a typo fails before anything is sent and a dry run names the device that would move. `inform_url` must be `http(s)://host[:port]/inform`: a wrong URL would leave the device reporting to nothing. Migrating an offline device returns a warning that it only receives the command on its next check-in. Local API only; in the `devices` profile. All three require `confirm` and support `dry_run`.

- **Tagged MAC management** (`list_mac_tags`, `get_mac_tag`, `create_mac_tag`, `update_mac_tag`, `delete_mac_tag`): full CRUD for device tags, the named MAC sets the controller uses to target WiFi broadcasts at a subset of APs, on the local gateway's legacy `rest/tag` endpoint. The Integration API only lists these (`list_device_tags`). MACs are validated and normalized to lowercase colon form. `update_mac_tag` takes `add_macs`/`remove_macs` and applies them to the stored members (read-modify-write), so an edit never drops members someone else added; `macs` replaces the list outright. Creating or renaming to a name that already exists (case-insensitive) is rejected rather than producing a duplicate. Writes re-read the tag when the controller does not echo it and attach `warnings` when the stored members differ from the request. Local API only; in the `devices` profile. Writes require `confirm` and support `dry_run`.
...
- **A2A HTTP endpoints were never served, and are now bearer-token protected**: on network transports `main()` logged `A2A endpoints: /a2a/agent-card, /a2a/discover, /a2a/delegate, /a2a/confirm, /a2a/audit`, then tried to mount the handlers onto an `app`/`_app`/`server`/`_server` attribute with `add_route` on the `FastMCP` instance. FastMCP 3.x has no such attribute, so every start logged `Could not auto-mount A2A routes onto FastMCP app` and every `/a2a/*` request returned 404. The routes are now registered through FastMCP's supported `custom_route` API (new `register_a2a_routes`). FastMCP's auth provider only guards `/mcp`, not custom routes, and `/a2a/delegate` and `/a2a/confirm` can trigger tool execution while `/a2a/audit` returns logged tool parameters. So all five routes, `agent-card` included, check the same `MCP_AUTH_TOKEN` bearer token as `/mcp` and answer `401` with `WWW-Authenticate: Bearer` without it. `stdio` registers no A2A routes. The self-asserted `auth` block in a delegate payload is still only used for A2A permission scoping; it is not an authentication substitute. `/a2a/delegate` still returns `"no tool executor configured"` because no executor is wired into `A2AState`.
- **`list_traffic_routes` always returned `[]` on controllers with Traffic Routes configured (issue #171, breaking)**: the four traffic-route tools targeted `/api/s/{site}/rest/routing` (which exists, but serves static routes) with a document shape — `action`, `match_criteria`, `dscp_marking`, `bandwidth_limit_kbps`, `priority` — that matches no UniFi resource, and the list filter kept only documents carrying `action` and `match_criteria`, which only this server's own writes ever had. The unrelated `200` was normalised to a clean empty list, so clients confidently reported that no policy routing (e.g. VPN routing) was configured. `list_traffic_routes` now reads UniFi's Traffic Routes from the local v2 endpoint `/proxy/network/v2/api/site/{site}/trafficroutes`, returns the real fields (`matching_target`, `target_devices`, `network_id`, `kill_switch_enabled`, `domains`, `ip_addresses`, `ip_ranges`, `regions`, `next_hop`; unknown keys preserved), requires `UNIFI_API_TYPE=local`, and raises on an unexpected response shape instead of returning `[]`. **Removed:** `create_traffic_route`, `update_traffic_route` and `delete_traffic_route` wrote that invented schema to the wrong resource; they will return on the v2 endpoint once the write path is verified on real hardware. The `RouteAction`, `MatchCriteria` and `RouteSchedule` models are removed; `TrafficRoute` is rebuilt around the v2 schema.
- **Every rejected sessionless request to `/mcp` leaked ~44 KiB, permanently (issue #173)**: the `mcp` SDK's `StreamableHTTPSessionManager` creates and registers a new session (transport plus server task) for any request without an `mcp-session-id` header *before* checking whether it is an `initialize` call, and with no idle timeout configured (FastMCP does not expose one) a session whose request was then rejected (`400` missing session, `406` bad `Accept`) was never released. A health check or uptime probe on `/mcp` once a minute grew RSS by ~54 MiB/day until the container thrashed. A new ASGI guard (`src/utils/session_guard.py`) on the `http` and `streamable_http` transports now answers sessionless requests that cannot start a session (anything other than an `initialize` POST) itself, with the same `400` the SDK would send, so no session is created; measured leak drops from ~45 KiB to ~0 per request. Unauthenticated requests are still answered `401` by the auth layer, and real clients (initialize, then session-scoped requests) are unaffected.
- **`list_integration_devices` failed on some hardware with a misleading "Invalid request parameters" (issue #170)**: `IntegrationDevice` typed `features` and `interfaces` as objects, which matches the detail endpoint, but the list endpoint can return just the names (`["switching", "accessPoint"]`, `["ports", "radios"]`) — as a UDR7 on firmware 5.1.33 does — so every device failed Pydantic validation. Both fields now accept either shape and keep it verbatim.
- **Cloud Connector passthroughs always returned `404 page not found`**: every `connector_network_*` and `connector_protect_*` call built its URL as `connector/{console_id}/proxy/network/{path}`, omitting the `consoles/` path segment the Site Manager API requires. The real endpoint is `https://api.ui.com/v1/connector/consoles/{console_id}/proxy/network/{path}` (and `.../proxy/protect/...`), which is the form already used throughout `docs/UNIFI_API.md`. Because the wrong path comes back as a bare 404 rather than an auth error, the failure reads like a bad key, an offline console, or an unsupported API — and the only way through was to smuggle the missing segment into `console_id` by hand (`console_id="consoles/<host-id>"`), which neither the docstrings nor `list_hosts` suggest. All ten passthroughs (Network and Protect, GET/POST/PUT/PATCH/DELETE) now build the `consoles/...` form, so `console_id` is the plain host id from `list_hosts`. Callers using the `"consoles/<id>"` workaround must drop the prefix, which would otherwise double the segment. The unit tests asserted the broken strings and were updated with them.
