...
### Fixed
- **Pre-commit ruff scanned the vendored `.agents/` skills**: `[tool.ruff] exclude` listed `.claude` but not `.agents`, so the pre-commit job linted the vendored mcp-builder scripts and failed on five D107/D103 findings in third-party code. The `lint` job never saw them because it runs `ruff check src/ tests/`. `.agents` now sits alongside `.claude` in the exclude list; `src/` and `tests/` linting is unchanged.
