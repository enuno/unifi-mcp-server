...
- **`get_application_info` (issue #108, item B5)**: requested `/integration/v1/application/info`, which no controller serves, so the tool 404'd everywhere. Now calls the documented `/v1/info` route and reports its `applicationVersion` as `application_version`, passing any additional keys a controller sends through verbatim.
