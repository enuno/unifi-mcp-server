...
async def set_ap_min_rssi(
    ...
    """Set the minimum client RSSI on one AP radio (config-record RMW).
    ...
    if radio is None:
        raise ValidationError(f"Unknown band '{band}'; use 2.4, 5, or 6")
    ...
    async with UniFiClient(settings) as client:
        ...
        stat_device = next(
            (
                d
                for d in devices
                if isinstance(d, dict) and (d.get("_id") == device_id or d.get("mac") == device_id)
            ),
            None,
        )
        if not stat_device:
